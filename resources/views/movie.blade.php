...
<div class = "container p-2 ml-5 mr-5">
    <div class="row align-items-end">
        <div class="col-lg-4 col-md-12 pl-5">
            <img src="data:image/png;base64, {{ $movie[0]->poster }}"
                width = 250 height = 340 class="shadow p-0 m-5 rounded" alt="...">
        </div>
        <div class="col-lg-8 col-md-12 pb-5">
            <h1 class = "pt-5">{{ $movie[0]->title }}</h1>
            <div class="row">
                <h4 class="float-left ml-3 mr-3"><span class = "badge badge-warning p-2">Metascore: {{ $movie[0]->metascore }}</span></h4>
                <h4 class="float-left mr-3"><span class = "badge badge-warning p-2">IMDb: {{ $movie[0]->IMDBrating }}</span></h4>
            </div>
        </div>
    </div>
    ...
    <div class="jumbotron jumbotron-fluid pl-5 pr-5 rounded" style = "margin-top: -12rem; padding-top: 12rem;">
        <div class="row mb-5">
            <div class="col-lg-4 col-md-12">
                <div> <span class = "font-weight-bolder"> PLOT:       </span>{{ $movie[0]->plot }}</div>
                <br>
                <div> <span class = "font-weight-bolder"> Awards:     </span>{{ $movie[0]->awards }}</div>                       
            </div>
            <div class="col-lg-4 col-md-12" >
                <ul class="list-group list-group-flush">
                    <li class="list-group-item">Rated:      {{ $movie[0]->rated }}</li>
                    <li class="list-group-item">Released:   {{ $movie[0]->released }}</li>
                    <li class="list-group-item">Runtime:    {{ $movie[0]->runtime }}</li>
                    <li class="list-group-item">Genre:      {{ $movie[0]->genre }}</li>
                </ul>
            </div>
            <div class="col-lg-4 col-md-12" >
                <ul class="list-group list-group-flush">
                    <li class="list-group-item">Language:   {{ $movie[0]->language }}</li>
                    <li class="list-group-item">Country:    {{ $movie[0]->country }}</li>
                    <li class="list-group-item">Director:   {{ $movie[0]->director }}</li>
                    <li class="list-group-item">Writer:     {{ $movie[0]->writer }}</li>
                </ul>
            </div>
        </div>
        <hr>
            ...
            @foreach ($cast as $role)
                <div class = "p-3">
                    ...
                    @else
                        ...
                        width = 160 height = 160 class = "img-fluid img-thumbnail p-lg-2 p-md-1 shadow" alt="">
                    @endif
                    <a href = "/TimRobbins" class = "btn-block pt-2 p-0 m-0 text-center">{{ $role->name }}</a>
                    <p class="p-0 m-0 text-center">plays</p>
                    <p class="p-0 m-0 text-center">{{ $role->role }}</p>
                </div>
            @endforeach
        </div>
        <div class = "p-5">
...
{{-- <div>
    ...
    @foreach ($cast as $role)
        {{ $role->name }}
        <img src="data:image/png;base64, {{ $role->headshot }}" alt="Poster Preview">
    @endforeach
    {{-- <img src="data:image/png;base64, {{ $dataURI }}" alt="Poster Preview"> --}} 
</div> --}}
@endsection